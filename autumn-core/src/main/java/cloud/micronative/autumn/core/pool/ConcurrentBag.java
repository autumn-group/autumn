...
public class ConcurrentBag<T extends ConcurrentBagEntry> implements AutoCloseable {
    ...
    private final ConcurrentLinkedQueue<T> sharedList = new ConcurrentLinkedQueue<>();
    ...
    public T borrow(long timeout, final TimeUnit timeUnit) throws InterruptedException {
        T entry = sharedList.poll();
        if(null != entry) {
            entry.compareAndSet(STATE_NOT_IN_USE, STATE_IN_USE);
            return entry;
        }
    ...
    public void add(T entry) {
        ...
        while (waiters.get() > 0 &&
                entry.getState() == STATE_NOT_IN_USE &&
